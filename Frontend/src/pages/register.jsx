...
const Register = (props) => {
    ...
    return (
        <div className={`min-h-screen h-full pb-8 flex flex-col items-center justify-baseline transition-all duration-300 ${theme === 'cyberpunk' ? 'cyberpunk-bg neon-text' : 'bg-gray-100 dark:bg-gray-900'}`}>
                <img onClick={() => navigate("/")} className="m-3 w-35 cursor-pointer" src="../../../public/kick.png" alt="" />
            <div className={`w-full max-w-md p-8 rounded-xl shadow-lg ${theme === 'cyberpunk' ? 'bg-black border-2 border-cyan-400 neon-text' : 'bg-white dark:bg-gray-800'}`}>
                <h2 className={`text-2xl font-bold mb-4 text-center ${theme === 'cyberpunk' ? 'text-cyan-400 neon-text' : 'text-gray-800 dark:text-white'}`}>Register</h2>
                <form>
                    {error && (
                            ...
                            {defaultAvatars.map((avatar) => (
                                ...
                                >
                                    <img
                                        src={avatar.src}
                                        alt={avatar.alt}
                                        className="w-10 h-10 sm:w-11 sm:h-11 md:w-12 md:h-12 rounded-full object-cover border border-white dark:border-gray-700"
                                    />
                                </div>
                            ))}
                            <div className=" cursor-pointer transition-all duration-200 transform hover:scale-105 flex-shrink-0 ring-1 ring-gray-200 hover:ring-indigo-300 rounded-full p-0.5 w-10 h-10 sm:w-11 sm:h-11 md:w-12 md:h-12 flex items-center justify-center bg-gray-100 dark:bg-gray-700 relative overflow-hidden ">
                                <input
                                    type="file"
                                    onChange={handleFileChange}
                                    className={`absolute inset-0 w-full h-full opacity-0 cursor-pointer ${theme === 'cyberpunk'?'bg-black':''} `}
                                />
                    ...
                    {(selectedAvatar != null || selectedFileAvatar != null) ? null : <label className={`block text-sm font-medium  ${theme === 'cyberpunk' ? 'text-pink-400 neon-text' : 'text-gray-700 dark:text-gray-200'} mb-3 text-center`}>
                        Choose a default avatar
                    </label>}

                    <div className="mb-4">
                        <label className={`block mb-2 text-sm font-medium ${theme === 'cyberpunk' ? 'text-pink-400 neon-text' : 'text-gray-700 dark:text-gray-200'}`} htmlFor="username">Username</label>
                        <input
                            type="text"
                            id="username"
                    ...
                    <div className="mb-4">
                        <label className={`block mb-2 text-sm font-medium ${theme === 'cyberpunk' ? 'text-pink-400 neon-text' : 'text-gray-700 dark:text-gray-200'}`} htmlFor="email">Email</label>
                        <input
                            type="email"
                            id="email"
                    ...
                    <div className="mb-6">
                        <label className={`block mb-2 text-sm font-medium ${theme === 'cyberpunk' ? 'text-pink-400 neon-text' : 'text-gray-700 dark:text-gray-200'}`} htmlFor="password">Password</label>
                        <input
                            type="password"
                            id="password"
