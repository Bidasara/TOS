...
function AnimationCard({ pack }) {
  ...
  // 1. Add new state for the break animation and a ref for the timeout
  ...
  const ANIMATION_DELAY = pack.pack.break.delay; // Delay in milliseconds (e.g., 300ms)
  ...
  // 2. Create new mouse event handlers with the delay logic
  const handleMouseEnter = () => {
    clearTimeout(waitForBreak.current);
    setCurrentAnimation('attack');
    setLoop(false);// Starts the sprite animation immediately

    // Start a new timeout to show the break animation after a delay
  ...
  const handleMouseLeave = () => {
    clearTimeout(waitForBreak.current);
    setShowBreakAnimation(false);
    clearTimeout(breakTimeoutRef.current); // IMPORTANT: Cancel the timeout
    setCurrentAnimation('idle');
    setLoop(true);
  };

  return (
    // 3. Use the new handlers
    <div
      onMouseEnter={handleMouseEnter}
      onMouseLeave={handleMouseLeave}
      className="flex-start flex lg:h-70 lg:w-850 sm:h-80 sm:w-150 bg-gray-100 rounded-md p-2"
    >
      <div className='w-1/2'>
          ...
          {!showBreakAnimation ? (
            ...
            >
              ...
              <div className="flex gap-2 items-center flex-shrink-0 mt-2 sm:mt-0">
                <button className="text-xs px-2 py-1 rounded bg-green-100 text-green-800 whitespace-nowrap">
                  Mark Revised
                </button>
                <a className="text-blue-600 hover:underline text-xs whitespace-nowrap">
                  View
                </a>
                <button className="p-1.5 rounded-full text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 dark:text-gray-500 dark:hover:text-indigo-400 dark:hover:bg-indigo-900/30">
                    ...
                    {animationPacks.map(pack => (
                        ...
                        >
                            ...
                            <div className="flex flex-col items-center md:items-start text-center md:text-left flex-grow">
                                ...
                                {/* CartButtons component needs to be included here */}
